...
The API and search runtime are unchanged from 1.3.2. Evercade, SNES PAL and generic clients continue to use the same module-v1 profiles, search aliases and Vinted detail-enrichment endpoints. Exact implementation-version matching is not required; consumers validate the API contract.

1.3.3 changes browser presentation only:
...
- the client-side Vinted background queue keeps the 1.3.2 three-item limit;
- card descriptions are cleaned and clamped only at render time; the API still returns the complete source description.
...
The full `description` remains available to API clients. The bundled 1.3.3 browser UI removes hashtag-only lines and trailing runs of three or more hashtag tokens from the visible card, then clamps the remaining text to four lines until the user selects `Mehr anzeigen`; this does not mutate the stored or returned listing.
...
- UI-only description cleanup does not alter API payloads, matching or scoring.
