"""GenericParser 0.42.7 standalone Free-plan bootstrap."""
...
from .build_identity_v0427 import API_CONTRACT, BUILD_ID, BUILD_REVISION, VERSION

SERVICE_MODULE = "generic_parser.search_service_v0427"
WORKER_UNIT = "standalone-bootstrap+free-cpu-work-packets"
...
def headers() -> dict[str, str]:
    return {
        ...
        "X-GenericParser-Bootstrap": "free-cpu-work-packets",
    }
...
async def search(request: Request) -> JSONResponse:
    try:
        ...
        return failure(request, status=400, detail="Ungültiger JSON-Request.", phase="request_json", exc=exc)
    try:
        service = load_service()
    except Exception as exc:
        return failure(request, status=500, detail="Free-Plan-Search-Service konnte nicht geladen werden.", phase="lazy_import_search_service", exc=exc)
    try:
        payload = service.SearchRequest.model_validate(body)
    except Exception as exc:
        return failure(request, status=422, detail="Suchrequest ist ungültig.", phase="search_request_validation", exc=exc)
    try:
        result = await service.search_page(payload, request)
    except Exception as exc:
        return failure(request, status=500, detail="Arbeitspaket konnte nicht verarbeitet werden.", phase="free_cpu_work_packet", exc=exc)
