const VERSION = "vinted-browser-poc-0.2.2";
...
function unwrapContent(raw) {
  try {
    ...
    if (parsed && parsed.success && typeof parsed.result === "string") return parsed.result;
  } catch {}
  return raw;
}
...
function extractHtmlListings(html) {
  ...
  while ((m = re.exec(html)) && listings.length < MAX_RESULTS) {
    ...
    const slugTitle = titleFromSlug(m[2] || "");
    ...
    const hrefMatch = m[0].startsWith("http") ? m[0] : `${BASE}${m[0]}`;
    listings.push({
      id: `vinted:${id}`,
      title: slugTitle || `Vinted Artikel ${id}`,
...
function extractTextListings(text) {
...
function extractListings(content) {
  ...
  return { strategy: "rendered-text", listings: extractTextListings(content) };
}
async function inspectVinted(env, query) {
  ...
  const sample = clean(content).slice(0, 1800);
  const lower = stripTags(content).slice(0, 5000).toLowerCase();
  ...
  const parsed = extractListings(content);
  ...
  return {
    ...
    listings,
    bodySample: status === "ok" ? undefined : sample
  };
}
export default { async fetch(request, env) {
  ...
  try {
    const result = await inspectVinted(env, normalizeQuery(url.searchParams.get("q")));
    return json(result, result.status === "ok" ? 200 : 502);
  } catch (error) {
    return json({ poc: VERSION, status: "error", reason: "browser_run_quick_action_failed", errorType: error?.name || "Error", error: String(error?.message || error) }, 500);
  }
}};
