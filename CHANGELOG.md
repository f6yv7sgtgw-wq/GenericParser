...
- Produktionsabnahme erfolgreich: Abnahmelauf `snes` (7050 Treffer in 494 Paketen, manuell beendet). **Der Wiedereinstieg ist belegt:** Die erste Vinted-Blockade wurde 67 s später vom Retry mit frischem Bootstrap wiedereröffnet (`retry: {attempt: 1}` im Quellenstatus, danach zwei weitere `ok`-Pakete, Budget korrekt zurückgesetzt). Die zweite Blockade blieb durch beide Anläufe (64 s, 63 s) bestehen — ehrliches Ende `blocked` nach 14 Paketen. Die Gesamtausbeute lag weiter bei ~250 Vinted-Treffern: das Volumenbudget wirkt offenbar **kumulativ über Bootstraps hinweg**; der Wiedereinstieg holt einzelne Pakete zurück, hebt die Decke aber nicht. Falls mehr Ausbeute nötig ist, wäre die längere Abklingzeit der nächste messbare Hebel (Roadmap).
- **Deploy-Flakiness bestätigt und behoben:** Der Pyodide-Download scheiterte zum zweiten Mal transient (`refused stream`, Läufe `31640299495` und `31675450576`) — beim zweiten Mal war der Upload bereits durch und nur die Live-Prüfungen liefen nie. Der Deploy-Schritt hat jetzt den in der Übergabe vorgesehenen Retry (3 Versuche); der Rerun von `31675450576` lief grün durch alle Live-Prüfungen. Rollback-Ziel: 1.9.1 / `gp-191-20260812-1`.
...
- Produktionsabnahme erfolgreich: Deploy-Workflow `31672063257` lief auf Commit `d3f55c8e128187dffc15513b9708044ae2dc1847` grün durch. Abnahmelauf `levis 501 w34 l34`: 4770 eindeutige Treffer in 235 Paketen ohne Retry. Kleinanzeigen endete ehrlich mit `source_complete` nach 5 Paketen; **eBay lief 219 Pakete bis zum echten natürlichen Ende** — der bisherige Eindruck, eBay ende bei ~25 Paketen, war derselbe Reporting-Effekt. Die Vinted-Pakete kamen gemessen im 25–27-s-Takt. **Neuer Befund:** Vinted blockierte trotz Schonfrist nach 10 vollen Paketen / exakt 250 Treffern — identisch zum Lauf ohne Schonfrist. Das Sitzungslimit ist volumenbasiert (~250 Treffer je anonymem Bootstrap), nicht ratenbasiert; der nächste Hebel ist ein erneuter Bootstrap, festgehalten im Übergabedokument. Rollback-Ziel: 1.9.0 / `gp-190-20260812-1`.
