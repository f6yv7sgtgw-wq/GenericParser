(() => {
  ...
  const LEGACY_KEYS = ['generic-parser-eventlog-04462', 'generic-parser-eventlog-04461', 'generic-parser-eventlog-0446'];
  const RECOVERY_KEY = I.autoResume?.recoveryKey || 'generic-parser-auto-resume-04463';
  ...
  const recoveryTitles = {
    ...
    recovery_probe_start: ['Recovery-Probe gestartet', 'Der vollständige Python-/ASGI-/Suchpfad wird geprüft.'],
    recovery_probe_failed: ['Recovery-Probe fehlgeschlagen', 'Die nächste Prüfung folgt nach dem gestaffelten Probe-Intervall.'],
    recovery_probe_ready: ['Suchpfad bereit', 'Worker, Python-Runtime, Search-Service und Referenzkern wurden erfolgreich geprüft.'],
  ...
  function presentation(event) {
    ...
    if (recoveryTitles[event?.type]) {
      ...
      const error = ['recovery_probe_failed','recovery_manual_required'].includes(event.type);
  ...
  async function verify() {
    ...
    try {
      ...
      box.innerHTML = [
        ...
        `<span>Recovery: ${recovery.enabled ? `${recovery.max_auto_resumes} Auto-Resumes · Probe ${esc(recovery.probe_endpoint || '?')}` : 'nicht aktiv'}</span>`,
  ...
  function render() {
    ...
    const legacyCount = data.filter(event => ['0.44.6','0.44.6.1','0.44.6.2'].includes(event.uiVersion)).length;
