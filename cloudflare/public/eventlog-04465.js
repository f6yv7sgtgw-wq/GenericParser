(() => {
  ...
  const LEGACY_KEYS = ['generic-parser-eventlog-04464','generic-parser-eventlog-04463','generic-parser-eventlog-04462','generic-parser-eventlog-04461','generic-parser-eventlog-0446'];
  ...
  function eventPresentation(event) {
    ...
    if (autoTitles[event?.type]) {
      const [title, fallback] = autoTitles[event.type];
      return {
  ...
  async function verify() {
    ...
    try {
      ...
      box.innerHTML = [
        ...
        `<span>Modus: sauberer Rollback auf ${esc(worker.operational_reference || '0.44.6.2')}</span>`,
        `<span>Diagnosemodus: ${referenceMode ? 'Referenz 0.44.4' : 'Standard'} · ${esc(schemaText)}</span>`,
        `<span>Automatische Fortsetzung: ${recovery.enabled ? `ein Versuch nach ${Math.round(Number(recovery.quiet_period_ms || 0) / 1000)} s Ruhezeit` : 'nicht aktiv'}</span>`
      ].join('');
    } catch (error) {
  ...
  document.getElementById('clear-log').onclick = () => {
    ...
    localStorage.removeItem('generic-parser-auto-resume-04465');
    render();
  };
