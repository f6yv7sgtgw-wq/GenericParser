(() => {
  ...
  const sourceUrl = new URL('./controller-0411.js?v=0.436-stable-source', location.href);
  fetch(sourceUrl, {cache: 'no-store'})
    ...
    .then(source => {
      ...
      Function(`${source}\n//# sourceURL=controller-0436-runtime.js`)();

      const controlledFetch = window.fetch;
      window.fetch = async function robustTitleFetch(input, init = {}) {
        ...
        if (/\/api\/search(?:\?|$)/.test(url)) {
          try {
            ...
            if (data?.coverage_diagnostics) {
              window.gpEventLog?.('coverage_diagnostics', 'Robuste Titel- und Trefferdiagnose', data.coverage_diagnostics);
            }
          } catch (_) {}
        }
        return response;
      ...
      if (state) {
        ...
        state.innerHTML = '<span><strong>Suchcontroller bereit</strong></span><span>Robuste Titelfallbacks aktiv: h2, Linktext, Attribute und Bild-alt.</span>';
      }
      window.GP_CONTROLLER_IDENTITY = {version: I.version, buildId: I.buildId, apiContract: I.apiContract, module: 'controller-0436.js'};
