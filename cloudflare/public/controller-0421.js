(() => {
  'use strict';
  const SOURCE='./controller-0411.js?v=0.421-source';
  const replacements=[
    ["const VERSION = '0.41.1';","const VERSION = '0.42.1';"],
    ["const BUILD_ID = 'gp-0411-20260802-1';","const BUILD_ID = 'gp-0421-20260802-1';"],
    ["const LOG_KEY = 'generic-parser-eventlog-0411';","const LOG_KEY = 'generic-parser-eventlog-0421';"]
  ];
  window.GP_CONTROLLER_READY=fetch(SOURCE,{cache:'no-store'})
    ...
    .then(source=>{
      ...
      Function(`${source}\n//# sourceURL=controller-0421-runtime.js`)();
      window.GP_CONTROLLER_VERSION='0.42.1';
      window.dispatchEvent(new CustomEvent('gp-controller-ready',{detail:{version:'0.42.1',buildId:'gp-0421-20260802-1'}}));
      return true;
    })
    .catch(error=>{
      ...
      if(text)text.innerHTML=`<span><strong>Controller 0.42.1 nicht geladen</strong></span><span>${String(error?.message||error)}</span>`;
      throw error;
    });
})();
