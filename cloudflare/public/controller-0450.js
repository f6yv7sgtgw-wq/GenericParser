(() => {
  ...
  const sourceUrl = new URL('./controller-0411.js?v=0.452-build5-reference-source', location.href);
  fetch(sourceUrl, {cache: 'no-store'})
    ...
    .then(source => {
      ...
      Function(`${source}\n//# sourceURL=controller-0452-build5-runtime.js`)();

      // Paid Worker mode: remove Free-tier pacing/protection delays while keeping retry count and search logic intact.
      ...
      if (delay) {
        delay.disabled = true;
        delay.innerHTML = '<option value="0" selected>0 Sekunden · Paid Worker</option>';
        delay.value = '0';
      }
      ...
      if (state) { state.className = 'compact-status done'; state.innerHTML = '<strong>Bereit</strong><span>Paid Worker · keine Schutzpausen · Modulvertrag v1 · Suche unverändert aus 0.44.6.5</span>'; }
