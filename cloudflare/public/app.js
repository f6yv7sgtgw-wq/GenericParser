...
function body(){
  ...
  if(maxResultsExplicit&&num('max-results')>0){add('max_results',num('max-results'));payload.max_results_explicit=true;} add('required_terms',list('required-terms')); add('excluded_terms',list('excluded-terms'));
  add('model_patterns',list('model-patterns')); add('brands',list('brands')); if(num('max-price')>0)add('max_price',num('max-price')); if(num('market-value')>0)add('market_value',num('market-value'));
...
$('profiles').addEventListener('change',e=>{if(!e.target.value)return;const d=JSON.parse(localStorage.getItem(e.target.value));Object.entries(d).forEach(([id,v])=>{if(!$(id))return;if($(id).type==='checkbox')$(id).checked=Boolean(v);else $(id).value=v});const savedLimit=String(d['max-results']??'').trim();maxResultsExplicit=savedLimit!==''&&Number(savedLimit)>0;$('max-results').dataset.explicit=String(maxResultsExplicit);$('profile-name').value=e.target.value.slice(11)});$('search-button').addEventListener('click',search);$('save-profile').addEventListener('click',saveProfile);$('max-results').addEventListener('input',e=>{if(e.isTrusted){maxResultsExplicit=e.target.value.trim()!=='';e.target.dataset.explicit=String(maxResultsExplicit);}});
const optionalText=['profile-name','required-terms','excluded-terms','model-patterns','brands','max-price','market-value','postal-code','location-id','radius-km','max-results']; optionalText.forEach(id=>$(id).value='');maxResultsExplicit=false;$('max-results').dataset.explicit='false'; $('query').value=''; $('token').value=localStorage.getItem('gp-token')||'';refreshProfiles();
window.addEventListener('pageshow',()=>{optionalText.forEach(id=>$(id).value='');maxResultsExplicit=false;$('max-results').dataset.explicit='false';setTimeout(()=>optionalText.forEach(id=>$(id).value=''),100);setTimeout(()=>optionalText.forEach(id=>$(id).value=''),400);});
if('serviceWorker'in navigator)navigator.serviceWorker.register("./service-worker.js").catch(()=>{});

$('demo-button').addEventListener('click',()=>msg('Demo entfällt in 0.3: Bitte eine Live-Suche oder ein gespeichertes Profil verwenden.'));
