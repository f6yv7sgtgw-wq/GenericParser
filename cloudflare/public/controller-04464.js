(() => {
  ...
  const sourceUrl = new URL('./controller-0411.js?v=0.4464-reference-source', location.href);
    ...
    .then(source => {
      const replacements = [
        ...
        [
          ...
          "const workerPhase = response.headers.get('X-GenericParser-Phase') || null;\n      const cfErrorType = response.headers.get('cf-error-type') || null;\n      const cfErrorOrigin = response.headers.get('cf-error-origin') || null;\n      const retryAfter = response.headers.get('retry-after') || null;\n      const responseRayId = response.headers.get('cf-ray') || null;\n      const bootstrapMode = response.headers.get('X-GenericParser-Bootstrap') || null;\n      log('after_fetch', 'Netzwerkantwort erhalten', {...common,status:response.status,ok:response.ok,elapsedMs,contentType,contentLengthHeader,workerVersion,workerBuild,workerContract,phase:workerPhase,cfErrorType,cfErrorOrigin,retryAfter,responseRayId,bootstrapMode});"
        ],
        [
      ...
      Function(`${source}\n//# sourceURL=controller-04464-runtime.js`)();

      const recoveryKey = String(I.autoResume?.recoveryKey || 'generic-parser-auto-resume-04464');
      ...
      if (state) { state.className = 'compact-status done'; state.innerHTML = '<strong>Bereit</strong><span>Referenz 0.44.4 · Lazy-Bootstrap-Recovery aktiv</span>'; }
      ...
      window.GP_CONTROLLER_IDENTITY = {version:I.version,buildId:I.buildId,apiContract:I.apiContract,module:'controller-04464.js',referenceVersion:'0.44.4',searchCoreChanged:false,autoResume:true,maxAutoResumes:2,recoveryProbe:true,resumeControlHotfix:true,lazyPackageBootstrap:true};
