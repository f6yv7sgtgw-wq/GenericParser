(() => {
  'use strict';
  const KEY='generic-parser-eventlog-0407';
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function rows(){try{return JSON.parse(localStorage.getItem(KEY)||'[]')}catch{return[]}}
  function render(){
    const data=rows().slice().reverse();
    document.getElementById('log-summary').innerHTML=`<span>${data.length} Ereignisse</span><span>${data[0]?new Date(data[0].time).toLocaleString('de-DE'):'Noch leer'}</span>`;
