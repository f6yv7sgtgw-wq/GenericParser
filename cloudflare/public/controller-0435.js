(() => {
  ...
  const sourceUrl = new URL('./controller-0411.js?v=0.435-stable-source', location.href);
  fetch(sourceUrl, {cache: 'no-store'})
    ...
    .then(source => {
      ...
      Function(`${source}\n//# sourceURL=controller-0435-runtime.js`)();

      const controlledFetch = window.fetch;
      window.fetch = async function titleEvidenceFetch(input, init = {}) {
        ...
        if (/\/api\/search(?:\?|$)/.test(url)) {
          try {
            ...
            if (data?.coverage_diagnostics) {
              window.gpEventLog?.('coverage_diagnostics', 'Treffer- und Titelstruktur der Quellseite', data.coverage_diagnostics);
            }
          } catch (_) {}
        }
        return response;
      ...
      if (state) {
        ...
        state.innerHTML = '<span><strong>Suchcontroller bereit</strong></span><span>Fehlende Titel werden mit begrenzten Strukturhinweisen protokolliert.</span>';
      }
      window.GP_CONTROLLER_IDENTITY = {version: I.version, buildId: I.buildId, apiContract: I.apiContract, module: 'controller-0435.js'};
