(() => {
  ...
  const sourceUrl = new URL('./controller-0411.js?v=0.4361-stable-source', location.href);
  fetch(sourceUrl, {cache: 'no-store'})
    ...
    .then(source => {
      ...
      Function(`${source}\n//# sourceURL=controller-04361-runtime.js`)();

      const controlledFetch = window.fetch;
      window.fetch = async function alignedDiagnosticsFetch(input, init = {}) {
        ...
        if (/\/api\/search(?:\?|$)/.test(url)) {
          try {
            ...
            if (data?.coverage_diagnostics) {
              window.gpEventLog?.('coverage_diagnostics', 'Titel-, Treffer- und Karteninformation', data.coverage_diagnostics);
            }
          } catch (_) {}
        }
        return response;
      ...
      if (state) {
        ...
        state.innerHTML = '<span><strong>Suchcontroller bereit</strong></span><span>Ergebniskarten zeigen Angebotsart, Titelquelle und Anzeigen-ID.</span>';
      }
      window.GP_CONTROLLER_IDENTITY = {version: I.version, buildId: I.buildId, apiContract: I.apiContract, module: 'controller-04361.js'};
