(() => {
  ...
  async function run(){setEnabled(false);render(false,'Identitätsprüfung läuft',[`${I.version} · ${I.buildId}`]);try{
    ...
    setEnabled(true);render(true,'Alle Codepfade konsistent',lines);log('deployment_handshake_ok','0.43.1 Single-Identity bestätigt',{checks,worker:d});
  }catch(e){setEnabled(false);render(false,'Deployment nicht konsistent',[String(e?.message||e)]);log('deployment_handshake_failed','Live-Suche wurde gesperrt',{error:String(e?.message||e)});}}
  run();
})();
