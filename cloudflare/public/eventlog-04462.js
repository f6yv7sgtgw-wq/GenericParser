(() => {
  ...
  function eventPresentation(event) {
    ...
    if (autoTitles[event?.type]) {
      ...
      return {
        className: event.type === 'auto_resume_manual_required' ? 'diagnostic error' : 'diagnostic done',
        title,
  ...
  function render() {
    ...
    const legacyCount = data.filter(event => ['0.44.6', '0.44.6.1'].includes(event.uiVersion)).length;
    ...
    summary.innerHTML = [
      ...
      legacyCount ? `<span>${legacyCount} Ereignisse aus 0.44.6/0.44.6.1 übernommen</span>` : '',
  ...
  document.getElementById('clear-log').onclick = () => {
    ...
    localStorage.removeItem('generic-parser-auto-resume-04462');
    render();
  };
